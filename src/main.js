...
function boot(){
  ...
  const wait=()=>{if(window.Cesium){new ShadowNexPrime().init().catch(fatal);}else if(Date.now()-started>15000){fatal(new Error('Map engine did not load. Check your connection and reload.'));}else setTimeout(wait,80)};wait();
}
