...
(()=>{
  ...
  const backdrop=document.createElement('div');backdrop.className='snx-help-backdrop';backdrop.innerHTML=`<section class="snx-help-panel" role="dialog" aria-modal="true" aria-labelledby="snxHelpTitle"><div class="snx-help-head"><div><div class="snx-help-kicker">SHADOWNEX PRIME™ · v2.2.0 DEMO & HELP</div><h2 id="snxHelpTitle">Global Situational Intelligence</h2></div><button class="snx-help-x" aria-label="Close help">×</button></div><p class="snx-help-sub">A quick walkthrough of the fastest way to navigate ShadowNex Prime and understand what each intelligence surface does.</p><div class="snx-help-video"><strong>60-second demo video slot ready.</strong><br>The written walkthrough is active now; a recorded app demo can be added later without rebuilding the Help system.</div><div class="snx-help-steps">${steps.map((s,i)=>`<div class="snx-help-step"><strong>${i+1}. ${s[0]}</strong><span>${s[1]}</span></div>`).join('')}</div><div class="snx-help-note"><strong>Data note:</strong> ShadowNex Prime combines public/open-source feeds and visualization tools. Feed health, timing and completeness vary by source; the interface should not be treated as classified, guaranteed real-time, or emergency-response data.</div><label class="snx-help-check"><input type="checkbox" class="snx-help-dont"><span>Don’t show this automatically again. The ? button always reopens Help.</span></label><button class="snx-help-done">ENTER SHADOWNEX</button></section>`;document.body.appendChild(backdrop);
