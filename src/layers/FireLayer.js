...
export class FireLayer extends BaseLayer{
 ...
 async refresh(){let points=[];let note='EONET';try{const r=await fetch('/api/fires');if(r.ok){const d=await r.json();if(d.configured&&Array.isArray(d.points)&&d.points.length){points=d.points;note='FIRMS';}}}catch{}
   ...
   this.clear();const C=window.Cesium;for(const p of points.slice(0,this.app.densityLimit(200,600,1200))){const meta={type:'FIRE',name:p.name||'Fire detection',latitude:p.lat,longitude:p.lon,brightness:p.brightness,when:p.when,source:note==='FIRMS'?'NASA FIRMS':'NASA EONET'};this.add({position:C.Cartesian3.fromDegrees(p.lon,p.lat,50),point:{pixelSize:this.app.compact?10:8,color:C.Color.ORANGERED,outlineColor:C.Color.YELLOW,outlineWidth:1},properties:{snxMeta:meta}});}this.setHealthy(points.length,note);}
}
